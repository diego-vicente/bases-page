...
function buildSortKeys(view) {
  if (view?.sort && view.sort.length > 0) {
    if (view.groupBy?.property) {
      return view.sort.filter((s) => s.property !== view.groupBy.property);
    }
    return view.sort;
  }
  if (view?.groupBy?.property) {
...
var bases_inline_default = `function f(e,n){let r=e.querySelectorAll(".bases-view-tabs button"),o=e.querySelectorAll(".bases-view");r.forEach(t=>{t.classList.toggle("is-active",t.dataset.viewIndex===String(n))}),o.forEach(t=>{t.classList.toggle("is-active",t.dataset.viewIndex===String(n))})}function v(e,n){let r=Number(e),o=Number(n);return!Number.isNaN(r)&&!Number.isNaN(o)?r-o:String(e).localeCompare(String(n))}function b(e,n,r){let o=e.querySelector("tbody");if(!o)return;let t=Array.from(o.querySelectorAll("tr"));if(r==="none"){let s=e._originalOrder;s&&s.forEach(i=>{o.appendChild(i)});return}e._originalOrder||(e._originalOrder=Array.from(t)),t.sort((s,i)=>{let a=s.children[n],l=i.children[n],c=a?.dataset?.value??a?.textContent??"",d=l?.dataset?.value??l?.textContent??"";return v(c,d)}),r==="desc"&&t.reverse(),t.forEach(s=>{o.appendChild(s)})}function m(e,n){e.querySelectorAll(".bases-table").forEach(o=>{let t=o.querySelectorAll("th[data-sortable='true']");t.forEach((s,i)=>{let a=()=>{let l=s.dataset.sortDirection||"none",c=l==="asc"?"desc":l==="desc"?"none":"asc";t.forEach(d=>{d!==s&&(d.dataset.sortDirection="none"),d.classList.remove("is-sorted-asc","is-sorted-desc")}),s.dataset.sortDirection=c,s.classList.toggle("is-sorted-asc",c==="asc"),s.classList.toggle("is-sorted-desc",c==="desc"),b(o,i,c)};s.addEventListener("click",a),n.push(()=>s.removeEventListener("click",a))})})}function E(e,n){let r=e.querySelectorAll(".bases-view-tabs button");if(r.length===0)return;let o=parseInt(e.dataset.initialView||"0",10);f(e,Number.isNaN(o)?0:o),r.forEach(t=>{let s=()=>{let i=parseInt(t.dataset.viewIndex||"0",10);f(e,Number.isNaN(i)?0:i)};t.addEventListener("click",s),n.push(()=>t.removeEventListener("click",s))})}function u(){let e=document.querySelectorAll(".bases-page");if(e.length===0)return;let n=[];e.forEach(r=>{E(r,n),m(r,n)}),window.addCleanup&&window.addCleanup(()=>{n.forEach(r=>{r()})})}document.addEventListener("nav",()=>{u()});document.addEventListener("render",()=>{u()});u();
`;
...
//# sourceMappingURL=chunk-KEWSXTFY.js.map
//# sourceMappingURL=chunk-KEWSXTFY.js.map