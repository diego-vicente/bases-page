...
var bases_inline_default = `function f(e,t){let s=e.querySelectorAll(".bases-view-tabs button"),n=e.querySelectorAll(".bases-view");s.forEach(r=>{r.classList.toggle("is-active",r.dataset.viewIndex===String(t))}),n.forEach(r=>{r.classList.toggle("is-active",r.dataset.viewIndex===String(t))})}function h(e){let t=String(e).trim();if(t==="")return{value:0,isStar:!1};if(/^[\u2605\xBD]+$/.test(t)){let s=0;for(let n of t)s+=n==="\\u2605"?2:1;return{value:s,isStar:!0}}return null}function p(e,t){let s=h(e),n=h(t);if(s&&n&&(s.isStar||n.isStar))return s.value-n.value;let r=Number(e),a=Number(t);return!Number.isNaN(r)&&!Number.isNaN(a)?r-a:String(e).localeCompare(String(t))}function v(e,t,s){return e.length===0||(e.sort((n,r)=>{let a=n.children[t],o=r.children[t],c=a?.dataset?.value??a?.textContent??"",i=o?.dataset?.value??o?.textContent??"";return p(c,i)}),s==="desc"&&e.reverse()),e}function g(e,t,s){let n=e.querySelector("tbody");if(!n)return;let r=Array.from(n.querySelectorAll("tr"));if(s==="none"){let o=e._originalOrder;o&&o.forEach(c=>n.appendChild(c));return}if(e._originalOrder||(e._originalOrder=Array.from(r)),r.some(o=>o.classList.contains("bases-table-group-header"))){let o=[],c=null;for(let i of r)i.classList.contains("bases-table-group-header")?(c={header:i,rows:[]},o.push(c)):c?c.rows.push(i):(c={header:null,rows:[i]},o.push(c));for(let i of o)v(i.rows,t,s);for(let i of o)i.header&&n.appendChild(i.header),i.rows.forEach(l=>n.appendChild(l))}else v(r,t,s),r.forEach(o=>n.appendChild(o))}function b(e,t){e.querySelectorAll(".bases-table").forEach(n=>{let r=n.querySelectorAll("th[data-sortable='true']");r.forEach((a,o)=>{let c=()=>{let i=a.dataset.sortDirection||"none",l=i==="asc"?"desc":i==="desc"?"none":"asc";r.forEach(u=>{u!==a&&(u.dataset.sortDirection="none"),u.classList.remove("is-sorted-asc","is-sorted-desc")}),a.dataset.sortDirection=l,a.classList.toggle("is-sorted-asc",l==="asc"),a.classList.toggle("is-sorted-desc",l==="desc"),g(n,o,l)};a.addEventListener("click",c),t.push(()=>a.removeEventListener("click",c))})})}function m(e,t){let s=e.querySelectorAll(".bases-view-tabs button");if(s.length===0)return;let n=parseInt(e.dataset.initialView||"0",10);f(e,Number.isNaN(n)?0:n),s.forEach(r=>{let a=()=>{let o=parseInt(r.dataset.viewIndex||"0",10);f(e,Number.isNaN(o)?0:o)};r.addEventListener("click",a),t.push(()=>r.removeEventListener("click",a))})}function d(){let e=document.querySelectorAll(".bases-page");if(e.length===0)return;let t=[];e.forEach(s=>{m(s,t),b(s,t)}),window.addCleanup&&window.addCleanup(()=>{t.forEach(s=>{s()})})}document.addEventListener("nav",()=>{d()});document.addEventListener("render",()=>{d()});d();
`;
...
//# sourceMappingURL=chunk-VTHYJCH5.js.map
//# sourceMappingURL=chunk-VTHYJCH5.js.map